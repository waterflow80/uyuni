...
def set_fake_file_data(package, files_count):
    ...
    TODO: can we fake the data in a better way
    """
    # We're setting fake data from real samples
...
class RPMMetadataParser:
    ...
    def parse_packages_metadata(self):
        ...
        for (
            ...
            parsed_filenames = self.filelists_parser.get_package_filelist(package["checksum"])["filenames"]
            parsed_filetypes = self.filelists_parser.get_package_filelist(package["checksum"])["filetypes"]
            if package["header"].get("filenames") and package["header"].get("filetypes") \
                    and len(package["header"].get("filenames")) == len(package["header"].get("filetypes")):
                # some file information has already been parsed from the Primary file
                # remove duplicates
                filenames = set(parsed_filenames)  # using hash set will be faster for searching in O(1) time
