...
def get_compatible_arches(channel_label):
    ...
    if not res_dict:
        logging.warning("Couldn't fetch compatible arches for channel: %s", channel_label)
        return None
    # pylint: disable-next=invalid-name
    with cfg_component("server.susemanager") as CFG:
        arches = [
            k["label"]
            for k in res_dict
            if CFG.SYNC_SOURCE_PACKAGES or k["label"] not in ["src", "nosrc"]  # TODO: what is CFG.SYNC_SOURCE_PACKAGES - ask team
        ]
    rhnSQL.closeDB()
    return arches
