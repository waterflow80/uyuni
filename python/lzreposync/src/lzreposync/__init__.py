...
from lzreposync.db_utils import (
    ...
    get_all_arches, create_channel, ChannelAlreadyExistsException,
)
...
# TODO: add more logs for easier debugging and understanding of what's happening
def main():
    ...
    if args.channel_info:
        ...
        print(f"Creating a new channel with label: {channel_label}, and arch: {channel_arch}")
        try:
            channel = create_channel(channel_label=channel_label, channel_arch=channel_arch)
            print(f"Info: successfully created channel: {channel_label} -> id={channel.get_id()}, name={channel.get_label()}")
